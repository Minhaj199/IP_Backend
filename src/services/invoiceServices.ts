...
export async function InvoiceCreation(
  inputItems: SelectItemType,
  totalAmount: number,
  customerName: string,
  ...
  try {
    ...
    const calculatedTotal = inputItems.reduce(
      (sum, item) => sum + item.total,
      0
    );
    if (calculatedTotal !== totalAmount)
      throw new Error("Total amount must match sum of item totals");
    ...
    const products: IproductDoc[] = await productModel
      .find({ _id: { $in: prodctIds } })
      .session(session);
...
export async function InvoiceDeletion(inputItems: SelectItemType, id: string) {
  ...
  try {
    ...
    const productId = inputItems.map((el) => ({
      _id: el.productId,
      stock: el.quantity,
    }));
